...
{
    EXPECT_CALL (db, swap ());
    EXPECT_CALL (db, enableAsynchronousVideoSync (Flip, _))
...
TEST_F(DoubleBufferTest, TestNoPaintedFullscreenOrFBOAlwaysBlitsSubBuffer)
{
    ...
    EXPECT_CALL (db, enableAsynchronousVideoSync (PartialCopy, _))
	    .WillOnce (Return (false));
    EXPECT_CALL (db, enableBlockingVideoSync (PartialCopy, _))
	    .WillOnce (Return (true));
    EXPECT_CALL (db, copyFrontToBack ()).Times (0);
...
TEST_F(DoubleBufferTest, TestSubBufferCopyIfNoFBOAndNoSubBufferBlit)
{
    ...
    EXPECT_CALL (dbStrict, enableAsynchronousVideoSync (PartialCopy, _))
	    .WillOnce (Return (false));
    EXPECT_CALL (dbStrict, enableBlockingVideoSync (PartialCopy, _))
	    .WillOnce (Return (true));
...
TEST_F(DoubleBufferTest, TestReportNoHardwareVSyncIfMoreThan5UnthrottledFrames)
{
	...
	EXPECT_CALL (db, enableAsynchronousVideoSync (PartialCopy, _))
		.WillOnce (Return (false));
	EXPECT_CALL (db, enableBlockingVideoSync (PartialCopy, _))
		.WillOnce (DoAll (SetArgReferee <1> (ExternalFrameThrottlingRequired),
				  Return (true)));
...
TEST_F(DoubleBufferTest, TestRestoreReportHardwareVSync)
{
    ...
    EXPECT_CALL (db, enableAsynchronousVideoSync (PartialCopy, _))
	    .WillOnce (Return (false));
    EXPECT_CALL (db, enableBlockingVideoSync (PartialCopy, _))
	    .WillOnce (DoAll (SetArgReferee <1> (FrameThrottledInternally),
			      Return (true)));
...
TEST_F (OpenGLVideoSyncTest, TestCallSwapIntervalOnEnableForFlipAndZeroForDisable)
{
    ...
    doubleBuffer.vsync (cgl::PartialCopy);
}
